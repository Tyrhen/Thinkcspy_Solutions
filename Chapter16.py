...
class rectangle:
    ...
    def contains(self,point_val):
        ...
        if (point_val.x >= self.bottom_leftcorner.x) and (
            point_val.x < self.bottom_leftcorner.x + self.width) and (
                point_val.y >= self.bottom_leftcorner.y) and (
                    point_val.y < self.bottom_leftcorner.y + self.height):
            return True
    ...
    def collision_detect(self,rectangle2):
        ...
        urc = point(rectangle2.bottom_leftcorner.x + rectangle2.width,
                    rectangle2.bottom_leftcorner.y + rectangle2.height)
        brc = point(rectangle2.bottom_leftcorner.x + rectangle2.width,
                    rectangle2.height)
        ulc = point(rectangle2.bottom_leftcorner.x,
                    rectangle2.bottom_leftcorner.y + rectangle2.height)
        
        #use the contains method on each point and return True if any evalulate to True
        if (self.contains(urc) == True) or (
            self.contains(ulc)== True) or (
                self.contains(brc)==True) or (
                    self.contains(blc) == True):
            return True
        else:
            return False
